...
export async function fetchMealsByUserAndDate(userId: string, date: string) {
  try {
    const response = await fetchApi(
      `/meal/getByUserIdAndDate?userId=${encodeURIComponent(
        userId
...
export async function deleteMealItem(mealId: string, userId: string) {
  ...
  try {
    const response = await fetchApi(
      `/meal/delete?mealId=${encodeURIComponent(
        mealId
...
export async function getDailyTotals(userId: string, date: string) {
  try {
    const response = await fetchApi(
      `/meal/getDailyTotal?userId=${encodeURIComponent(
        userId
      )}&date=${encodeURIComponent(date)}`
    );
    if (!response.ok) {
...
export async function getTypeTotals(userId: string, date: string) {
  try {
    const response = await fetchApi(
      `/meal/getTypeTotals?userId=${encodeURIComponent(
        userId
      )}&date=${encodeURIComponent(date)}`
    );
    if (!response.ok) {
