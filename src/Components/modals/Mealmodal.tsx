...
export const amount = ["ml", "g", "pieces"];
...
export default function Mealmodal({
  ...
  const handleSubmit = async () => {
    if (!input.name.trim() && !input.mealtype && !input.weight) {
      setFeedback({
        message: "Please fill in  name, weight and mealtype before submitting",
        type: "error",
      });
    } else if (!input.name.trim() && !input.mealtype) {
      setFeedback({
        message: "Please fill in name and mealtype before submitting",
        type: "error",
      });
    } else if (!input.name.trim() && !input.weight) {
      setFeedback({
        message: "Please fill in name and weight before submitting",
        type: "error",
      });
    } else if (!input.name.trim()) {
      setFeedback({
        message: "Please fill in name before submitting",
        type: "error",
      });
    } else if (!input.weight && !input.mealtype) {
      setFeedback({
        message: "Please fill in mealtype and weight before submitting",
        type: "error",
      });
    } else if (
      ...
      setFeedback({
        message: "Please fill in mealtype before submitting",
        type: "error",
      });
    } else {
      try {
        ...
        setFeedback({
          message: "MealItem added successfully!",
          type: "success",
        });
        setSelectedMealId("");
  ...
  const filterOFFDataByCountry = (
    productlist: OFFMealItem[],
    countries: string[]
  ) => {
  ...
  const handleSubmitInputOFFAPI = async (
    ...
    if (searchTerm != null) {
      getFoodFromFoodFactsAPI(searchTerm)
        .then((data) =>
          filterOFFDataByCountry(data.products || [], relevantCountries)
        )
  ...
  useEffect(() => {
    fetchMealsByUser(user.userId).then(setData).catch(console.error);
    console.log(data);
    if (!isOpen) {
  ...
  return (
    <div
      className="fixed inset-0 backdrop-blur-sm bg-white/1 flex items-center justify-center z-50"
    ...
    >
      <div className="bg-white rounded-lg p-6 relative shadow-lg min-w-[300px] max-w-[35vw] w-full">
        ...
        <form onSubmit={handleSubmitInputOFFAPI}>
          ...
          <button
            type="submit"
            className="px-4 py-2 p-2 mt-2 bg-blue-500 text-white rounded hover:bg-blue-600"
          >
            Search
          </button>
        </form>
        <div className="mb-4 flex flex-col space-y-4 font-bold font-serif text-sm">
          <form action="">
            ...
            >
              <option value="">Select from food you previously added</option>
              {data.map((food) => (
                <option value={food.mealId} key={food.mealId}>
                  {food.name} ({food.date})
                </option>
              ))}
            </select>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (field === "weight") {
                          ...
                          else if (selectedUnit === "pieces")
                            handleChange("pieces", val);
                        } else {
